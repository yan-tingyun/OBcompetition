...
RC FieldMeta::init(const char *name, AttrType attr_type, int attr_offset, int attr_len, bool visible, int is_null) {
...
RC FieldMeta::from_json(const Json::Value &json_value, FieldMeta &field) {
  ...
  if (!is_null_value.isInt()) {
    LOG_ERROR("nullable field is not a int value. json value=%s", is_null_value.toStyledString().c_str());
    return RC::GENERIC_ERROR;
  }
  ...
  int is_null = is_null_value.asInt();
