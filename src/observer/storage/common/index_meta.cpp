...
RC IndexMeta::init(const char *name, const FieldMeta &field, const int is_unique) {
...
RC IndexMeta::from_json(const TableMeta &table, const Json::Value &json_value, IndexMeta &index) {
  ...
  if (!is_unique_value.isInt()) {
    LOG_ERROR("is_unique is not an integer. json value=%s", is_unique_value.toStyledString().c_str());
    return RC::GENERIC_ERROR;
  }
  ...
  return index.init(name_value.asCString(), *field, is_unique_value.asInt());
}
