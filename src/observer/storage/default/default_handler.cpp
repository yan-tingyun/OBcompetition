...
RC DefaultHandler::create_multi_index(Trx *trx, const char *dbname, const char *relation_name, const char *index_name, const RelAttr *attr_list, int attr_num, int is_unique) {
  Table *table = find_table(dbname, relation_name);
  if (nullptr == table) {
    return RC::SCHEMA_TABLE_NOT_EXIST;
  }
...
RC DefaultHandler::insert_record(Trx *trx, const char *dbname, const char *relation_name, int value_num, const Value *values, int record_num, const int record_pos[]) {
  Table *table = find_table(dbname, relation_name);
  if (nullptr == table) {
    return RC::SCHEMA_TABLE_NOT_EXIST;
  }

  return table->insert_record(trx, value_num, values, record_num, record_pos);
}
